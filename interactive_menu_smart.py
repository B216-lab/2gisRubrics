#!/usr/bin/env python3
"""
ИНТЕРАКТИВНОЕ МЕНЮ - со СМАРТ-ОБНАРУЖЕНИЕМ разделителя

АВТОМАТИЧЕСКИ ОПРЕДЕЛЯЕТ:
- Разделитель CSV (запятая или точка с запятой)
- Названия столбцов (даже если они не совпадают)
...
def find_columns(df):
    """Находит нужные столбцы в DataFrame"""
    id_col = None
    name_col = None
    desc_col = None
    ...
    for col in df.columns:
        ...
        if col in ['№', 'N', 'ID', 'id', 'Num'] or 'id' in col_lower or '№' in col:
            id_col = col
        elif col in ['Тип', 'Type', 'Category', 'Название', 'Name'] or 'тип' in col_lower or 'name' in col_lower:
            name_col = col
        elif col in ['Общее описание', 'Description', 'Desc', 'Описание'] or 'описание' in col_lower or 'desc' in col_lower:
            desc_col = col
    
    # Fallback: если не нашли, берём по позиции
    ...
    return id_col, name_col, desc_col

class RubricsAppSmart:
    def __init__(self):
    ...
    def load_categories(self):
        """Загрузка категорий из файла (с СМАРТ-обнаружением)"""
        print("\n" + "="*80)
        ...
        try:
            if file_path.endswith('.csv'):
                # СМАРТ-обнаружение разделителя
                separator = detect_csv_separator(file_path)
                print(f"  ℹ️  Обнаружен разделитель: {repr(separator)}")
                
                df = pd.read_csv(file_path, encoding='utf-8', sep=separator)
                df.columns = df.columns.str.strip()
                
                # СМАРТ-поиск столбцов
                id_col, name_col, desc_col = find_columns(df)
                print(f"  ℹ️  Найденные столбцы:")
                print(f"      - ID: {id_col}")
    ...
    def load_rubrics(self):
        """Загрузка рубрик из файла (с СМАРТ-обнаружением)"""
    ...
    def run(self):
        """Главное меню"""
        print("\n" + "="*80)
        print("СИСТЕМА КЛАССИФИКАЦИИ РУБРИК 2ГИС (SMART VERSION)")
        print("="*80)
        print("\nИнтерактивное меню с автоопределением формата файлов")
        print("Поддерживает CSV с любым разделителем (,  или ;)")
